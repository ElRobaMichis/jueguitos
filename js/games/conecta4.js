    ...
    for(let y = 0; y < H; y++){
      for(let x = 0; x < W; x++){
        ...
        board.append(el('button', {
          class: 'cell c4-cell' + (s.win?.line.includes(i) ? ' win' : '') + (s.last === i ? ' last' : ''),
          onclick: () => {
            if(s.win || !P.isMe(s.turn)) return;
            beep(520, .07); vibrate(18);
            api.act({ x });
          },
        }, v ? el('span', { class:'disc', style:{ background:P.color(v) } }) : ''));
      }
    }
